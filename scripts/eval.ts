...
// ---------------------------------------------------------------------------
// Constants — mirroring src/lib/codewise.functions.ts
// ---------------------------------------------------------------------------

const VALID_TOPIC_SLUGS = new Set([
  "arrays", "strings", "hashing", "linked-lists", "stacks", "queues",
  "recursion", "two-pointers", "sliding-window", "binary-search", "sorting",
  "trees", "bst", "heaps", "graphs", "dp", "greedy", "backtracking",
...
async function callAI(code: string, language: string): Promise<z.infer<typeof ReviewResponseSchema>> {
  ...
  return ReviewResponseSchema.parse(JSON.parse(content));
}
